...
\begin{table}[h]
	...
	{\renewcommand{\arraystretch}{1.1}% para el espaciado vertical
		...
		\begin{tabular}{p{2.5cm}p{2.5cm}p{2cm}p{5.5cm}}
			...
			\cite{bravi2023transfer}	&	diffRBM	&	BLOSUM62	 &	Para el modelo de presentación alelo-específico de inmunogenicidad, emplea modelos con transferencia de aprendizaje y Máquinas de Boltzmann Restringidas.	\\%El modelo utiliza BLOSUM62
			
			\cite{10.1093/bioinformatics/btac820}	&	AVIB	&	BLOSUM50	&	Utiliza Atención de Expertos (AoE). Utiliza multiatención para predecir las interacciones entre TCRs y péptidos.	\\
			
			\cite{myronov2023bertrand}	&	BERTrand	&	BLOSUM62	&	Emplea un modelo BERT con 8 bloques de transformadores y más de 2.5 millones de parámetros, y utiliza una estrategia de preentrenamiento no supervisado para compensar la cantidad de parámetros.\\ %La entrada se representa mediante tokens, se convierte en una incrustación y se pasa a un cabezal de clasificación.
			
			\cite{zhang2023hybrid}	&	Hybrid gMLP 	&	BLOSUM50	&	El gMLP basado en el aprendizaje profundo utiliza múltiples mecanismos de atención para predecir la interacción de péptidos MHC y TCR, y puede manejar los problemas causados por las diferentes longitudes de los TCR.\\
			
			\cite{zhang2022pite}	&	PiTE	&	BLOSUM	&	Utiliza un modelo de incrustación de aminoácidos y un codificador de secuencias. Las representaciones se concatenan con su resta absoluta y se alimentan a dos capas lineales.
			%para obtener la representación u y v de la secuencia TCR y el epítopo. Finalmente, u, v y su resta absoluta $|u-v|$ se concatenan y se alimentan a dos capas lineales para predecir la afinidad de unión.
			\\
			
			%2023\cite{wu2023tpbte}	&	TPBTE	&	BLOSUM62	&		\\%No está el artículo
			
			\cite{yang2023mix}	&	MIX-TPI	&	BLOSUM62	&	Emplea CNNs con autoatención para construir un extractor basado en secuencias y características fisicoquímicas. Luego las fusiona con una capa de autoatención para predecir las interacciones TCR-péptido.\\
			
			\cite{bradley2023structure}	&	TCRdock	&	BLOSUM62	&	Una versión especializada de AlphaFold para generar modelos de interacciones TCR:péptido-MHC que se pueden utilizar para distinguir los epítopos de los incorrectos. 	\\
			
			\cite{fang2022attention}	&	ATMTCR	&	BLOSUM50	&	ATMTCR se alimenta en dos capas completamente conectadas, un modelo basado en el aprendizaje por contraste y NetMHCpan para predecir la unión del TCR y el complejo pMHC.
			\\
			
			\cite{cai2022atm}	&	ATM-TCR	&	One-hot	\& BLOSUM &	Consta de dos codificadores y un decodificador lineal. % para determinar la afinidad de unión.
			...
			\cite{xu2022attntap}	&	AttnTAP	& Vectores de Granularidad	&	Red de aprendizaje profundo de doble entrada que incluye una red LSTM bidireccional, un mecanismo de atención y una perceptrón multicapa para extraer las características del TCR y el péptido y predecir la unión TCR-péptido.\\
			...
			\cite{9994875}	&	pMTattn	&	Incrustación	&	Emplea un mecanismo de atención cruzada para aprender información de interacción entre pMHCs y TCRs. Para codificar cada uno de ellos en una incrustación, utiliza modelos basados en transformadores.
			\\
			
			\cite{xu2021dlptcr}	&	DLpTCR	&	One-hot	&    DLpTCR consiste en FCN, LeNet-5 y ResNet-20 para predecir el péptido-CDR3$\alpha$($\beta$) y una estrategia de ensamble multi-modelo. %para obtener resultados más fiables.
			También implementa un mecanismo de atención en ResNet-20.% para mejorar la calidad de las salidas generadas.
			\\
			
			\cite{fischer2020predicting}	&	TcellMatch	&	BLOSUM50	&	Employs embeddings for amino acids and a sequence embedding block composed by multiple layer types.
			%: recurrent layers, self-attention, convolutional layers and densely connected layers.
			The activation generated is fed into a dense network to predict binding events. \\
